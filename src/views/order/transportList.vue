    ...
    <el-table
      ...
      <el-table-column align="center" label="操作">
        <template slot-scope="scope">
          ...
          <el-tooltip class="item" effect="dark" content="订单完成" placement="bottom">
            <a class="el-icon-success" style="margin-right:10px" @click="confirmReceive(scope.row.orderNo)"/>
          </el-tooltip>
        </template>
      </el-table-column>
    </el-table>
    ...
    confirmReceive(orderNo) {
      this.$confirm('是否确认订单完成', '提示', {
        ...
        .then(() => {
          ...
          api.transportComplete(params).then(response => {
            this.fetchData()
          })
          this.$message({
            type: 'success',
